...
class TestNotesHistory:
    ...
    def test_history_route_renders_in_configured_tz(self, client: TestClient, monkeypatch):
        """updated_at is stored as naive UTC; the route converts it to the
        deployment's configured timezone and emits a TZ-tagged string. Configured
        for Pacific here; TestNotesHistoryTimezone covers a non-Pacific region."""
        ...
        assert response.status_code == 200
        text = response.text
        # Configured-PT abbreviation appears as PDT or PST depending on the date.
        assert " PDT" in text or " PST" in text, f"configured-TZ suffix missing — UTC bleed-through? {text[:120]!r}"
...
class TestRegenerateConfirm:
    def test_returns_modal_with_last_prep_timestamp(self, client: TestClient, monkeypatch):
        """Modal for stage=materials_drafted surfaces last-completion timestamp
        formatted in the configured TZ, points Confirm at /regenerate and Cancel at
        /regenerate/cell."""
        monkeypatch.setenv("TZ", "America/Los_Angeles")
        ...
        # Timestamp rendered in the configured TZ (UTC 22:15 -> PDT 15:15)
...
class TestNotesHistoryTimezone:
    """notes_history renders timestamps in the deployment's configured timezone,
    not a hardcoded Pacific zone (#967 regionalization sweep)."""

    def test_times_follow_configured_tz_not_hardcoded_pacific(self, client, monkeypatch):
        # Seed a note at a known UTC instant. 02:30 UTC Jun 1 == 11:30 JST Jun 1
        # (UTC+9) but 19:30 PDT May 31 — the date AND zone differ, so a hardcoded
        # Pacific conversion is distinguishable from the configured one.
        conn = sqlite3.connect(client._db_path)
        conn.execute(
            "INSERT INTO notes_history (job_id, notes, updated_at) VALUES (?, ?, ?)",
            ("id_scored", "ping", "2026-06-01 02:30:00"),
        )
        conn.commit()
        conn.close()

        monkeypatch.setenv("TZ", "Asia/Tokyo")
        r = client.get("/board/jobs/fp_scored/notes/history")
        assert r.status_code == 200
        assert "2026-06-01 11:30 JST" in r.text, "time must render in the configured TZ (JST)"
        assert "PDT" not in r.text and "PST" not in r.text, "must not hardcode Pacific"
