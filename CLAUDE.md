...
<repo>/src/findajob/web/routes/onboarding_interview.py # In-app interview routes: /onboarding/interview/start | /turn | /{sid} | /{sid}/finalize. _resolved_chat_key precedence: OPENROUTER_OPERATOR_KEY (subsidy) > tester key > 503. Step 1 keys mandatory before /start.
...
- **Step 2 — Run the interview.** Disabled until Step 1 succeeds. Tester runs the entire interview as a chat inside findajob's UI. Server-side persistent — close the tab and reload to resume. Routes live in `findajob.web.routes.onboarding_interview`; runtime-gate per request via `_resolved_chat_key`. Chat funding precedence: `OPENROUTER_OPERATOR_KEY` env (subsidy on findajob-test / operator-deployed-for-tester) > tester's collected OpenRouter key. The pipeline (triage, scoring, prep) always runs on the tester's own key from Step 1; the operator key only subsidizes the interview chat. ~$0.50 per onboarding for Sonnet 4.6 with prompt caching (system-prompt cache_control breakpoint).
