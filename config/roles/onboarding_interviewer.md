...
- For `timezone.txt`: write a comment line into `data/timezone`. The first
  non-comment line is read at container boot and exported as `TZ` (#981), so a
  stray comment line is harmless but the value line must be a clean IANA zone.
