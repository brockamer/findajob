...
Extracted from `scripts/prep_application.py` in M3 (#537). A byte-equivalent
copy still lives in `scripts/interview_prep.py`; the next M3 PR moves that
into `findajob.interview.role_runner`. Both copies will then be consolidated
into `findajob.llm.role_runner` in the M3 cleanup PR (or M3+).
"""
