...
- **briefing_writer upgraded to Opus 4.7 (#264).** `briefing_writer` was set to Sonnet 4.6 in the #250 OpenRouter Phase 2 cutover; that briefing cascades into `resume_tailor` and `cover_letter_writer`, both already on Opus 4.7. Promoting the briefing to the same tier closes the quality cascade — every downstream artifact now reads from a top-tier briefing rather than a Sonnet-tier one. Cost delta: ~$0.10 → ~$0.50 per briefing call (~$2/day at 5 preps/day on the operator stack). No migration required.
