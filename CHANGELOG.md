...
- **Per-tester key migration is operational, not automatic.** #339 delivers the capability for new tester stacks to carry only their own credentials. Existing tester stacks (alice, papa, etc.) keep running on the operator's keys until rotated explicitly — the `sed -i` rotation recipe in `docs/setup/install-docker.md` § "Operating an existing stack → Rotating an API key" is the operator-side migration path. Until that runs, the "no shared/operator keys leak" property is a forward-looking floor for new stacks, not a retroactive guarantee for existing ones.
