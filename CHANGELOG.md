...
### Refactor
- **#470 / Phase 1 of the OpenRouter Native Migration (#469):** added `src/findajob/llm/openrouter.py` — stdlib HTTP wrapper for OpenRouter chat completions. Public API: `complete()`, `CompletionResult`, typed `OpenRouterError`. Supports Anthropic `cache_control` on two axes (`cached_prefix` for shared content placed in the user message, `cache_system` for sufficiently large system prompts), provider pinning via `pin_provider`, retry-by-shape on 429/5xx/network (jittered exponential backoff, 3 attempts max), and direct `cost_log.cost_usd` writes from `response.usage.cost` via the new `cost_usd_override` keyword on `findajob.cost_tracking.log_call`. Ported `job_scorer` (highest call volume, simplest prompt) as proof-of-concept; the cached_prefix split (profile + feedback_block stable, JD/title/company per-call) sets the pattern for Phase 2's Opus-using ports where caching does cut cost (job_scorer's DeepSeek does not honor cache_control). **Parallel coexistence:** aichat-ng still drives 7/8 call sites — no operator-visible behavior change in this release. Phase 2 (#471) ports the remaining sites; Phase 3 (#472) deletes aichat-ng + retires the calibration stack.
- **#471 OpenRouter Native Migration Phase 2** — All remaining LLM call sites (briefing_writer, resume_tailor, cover_letter_writer, recruiter_critic, fit_analyst, company_researcher, resume_change_reviewer, outreach_drafter, interview_prep, company_discoverer, candidate_led_briefing, speculative_roles_synth, voice_processor, onboarding interview) now route through `findajob.llm.openrouter.complete()`. Every `cost_log` row carries API-authoritative `usage.cost`. The `aichat-ng` binary is still bundled (Phase 3 retires it). The 4 prep-stage Opus calls + outreach_drafter + interview_prep + speculative_roles_synth + onboarding interview pass `pin_provider="anthropic"` so same-role calls hit the prompt cache. Cross-role caching across the prep chain is deferred to #478 (the prep-chain restructuring is a separate epic). Closes #463 — onboarding interview turns now write `cost_log` rows via the canonical wrapper path (route handler reuses `add_turn_cost`'s connection to write the API-authoritative cost+tokens trio). Note: `speculative_roles_synth` uses `cached_prefix=profile+master_resume` (not `profile+briefing` as the plan first specified) — briefing differs per speculative request, so including it in the prefix would produce 0% cache hits.

### Added
- **Real-time cost visibility — nav credits chip, dashboard burn-rate widget, per-job cost cell, monthly projection (#87).** Five UI surfaces now read calibrated cost numbers from a new `cost_calibration` table populated every 5 min by `scripts/poll_openrouter_credits.py`: a credits-remaining chip on every page (replacing the legacy onboarding-cost chip), a three-line burn-rate widget on the dashboard (this week / projected month / runway weeks), a per-job cost cell on the Applied tab, a per-operation breakdown on the Materials view, and a calibrated-monthly projection line in the 06:15 `notify-stats` ntfy. Calibration multiplier is computed from a rolling 7-day window (#467): `(delta_credits_used - delta_onboarding_cost) / heuristic_sum_7d`, clamped to [0.5, 3.0] — comparing window-matched deltas instead of lifetime cumulatives so the math reflects pipeline-driven spend over the same period as the heuristic. Each stack starts with `poll_status='warming_up'` and `multiplier=1.0` until 7 days of poll history accumulate; sparse weeks (no prep activity) inherit the last good multiplier. The onboarding subtraction is a workaround pending #463 (which retires the dual-table cost tracking).

### Changed
- **Replaced onboarding-only nav-bar cost chip with credits-remaining chip (#87).** The legacy `$X.XX onboarding` badge is retired; onboarding cost remains visible on the post-onboarding result page only.
- **`notify-stats` 7d-spend section now uses calibrated heuristic numbers (#87).** Disclaimer updated from "accurate to ~±30% vs actual bills" to "Calibrated every 5 min against OpenRouter; numbers track account spend within ~5%" on stacks with calibration data; falls back to "Uncalibrated — no calibration data yet" on legacy stacks.

### Migration required
- New table `cost_calibration` is created idempotently by `init_db.py` on container boot — no operator action.
- New cron job `poll-openrouter-credits` runs every 5 min via supercronic. Stacks with `OPENROUTER_API_KEY` set in `data/.env` see calibrated cost numbers; stacks without the key see uncalibrated numbers (no errors, no chip rendered).
