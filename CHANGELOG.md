...
### Migration required
- **`docker compose pull && docker compose up -d` is the entire migration path.** Existing stacks: `migrate_schema()` in `init_db.py` idempotently `DROP TABLE cost_calibration` on first boot post-upgrade; the every-5-min `poll-openrouter-credits` cron entry is removed from `crontab` automatically. Fresh installs never see the table.
- **`state/aichat_ng/` bind mount is no longer used.** After the container is healthy, operators may `rm -rf state/aichat_ng/` to reclaim disk; the path is no longer referenced by `compose.yaml.example`. Leaving it in place is harmless.
